...
parser.add_argument('--learn_rate', '-l', type=float, default=0.001)
...
def train(model, dataloader, test_dataloader, epoch, criterion, optimizer, scaler):
    ...
    for i in range(epoch):
        ...
        if test_loss.data < max_loss:
            torch.save(model.state_dict(), "./model/Mul_GridLSTM_SML.pt")
...
def test(model, test_dataloader, criterion):
    model.load_state_dict(torch.load("./model/Mul_GridLSTM_SML.pt"))
    model.eval()
    for (test_x, test_target) in test_dataloader:
        test_x = test_x.permute(1, 0, 2)
        test_x = test_x.to(device)
...
# RMSE:  0.018341292159306185 | R2: 0.9996667610523186 | MAE: 0.012777969 | MAPE : 0.00788345983472921
...
# RMSE:  0.019049763231539868 | R2: 0.9996441422305988 | MAE: 0.012485542 | MAPE : 0.007814926904858457
# RMSE:  0.018341292159306185 | R2: 0.9996667610523186 | MAE: 0.012777969 | MAPE : 0.00788345983472921